...
const DSLapDuThao = () => {
  ...
  // Dữ liệu giả lập
  const mockData = [
    { stt: 1, tenDN: 'Chi nhánh công ty CP Xăng dầu dầu khí PV Oil Miền Trung tại Đà Nẵng', loai: 'Cấp sửa đổi bổ sung', ngay: '02/01/2026', trangThai: 'Chờ lập dự thảo' },
    { stt: 2, tenDN: 'Chi nhánh Công ty TNHH Dũng Sông Hàn - Cửa hàng kinh doanh xăng dầu', loai: 'Cấp mới', ngay: '04/02/2026', trangThai: 'Đã lập dự thảo' },
    { stt: 3, tenDN: 'Công ty TNHH MTV Hòa Khánh 1', loai: 'Cấp sửa đổi bổ sung', ngay: '05/02/2026', trangThai: 'Chờ lập dự thảo' },
    { stt: 4, tenDN: 'Công ty TNHH TM&DV Bách An Nam', loai: 'Cấp lại', ngay: '14/02/2026', trangThai: 'Chờ lập dự thảo' },
  ];

  return (
    <div className="h-full flex flex-col">
      <h1 className="text-xl font-bold text-gray-800 uppercase text-center mb-8 tracking-wide">
        Danh sách hồ sơ chờ lập dự thảo
      </h1>
            ...
            {mockData.map((row, index) => (
              <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-4 text-center border-r border-gray-200 font-bold">
                  <span className={row.trangThai === 'Đã lập dự thảo' ? 'text-green-600' : 'text-orange-500'}>
                    {row.trangThai}
                  </span>
                </td>
                <td className="px-4 py-4 text-center">
                  <button 
                    // Chuyển hướng sang trang chi tiết (LapDuThao.tsx)
                    onClick={() => navigate('/lap-du-thao/chi-tiet')} 
